...
public class TotalAmount {
	...
	public TotalAmount() {
		...
		this.availableMeals = new ArrayList<>(Arrays.asList(100, 100, 100, 100, 100, 100));
		this.mealsNames = new ArrayList<>(Arrays.asList("Macarrones con Queso", "Carbonata", "Pizza", "Fetuccini",
				"Frutti di Mare", "Carapaccio"));
		this.priceMeals = new ArrayList<>(Arrays.asList(5, 8, 10, 35, 50, 100));
		this.specialMenuID = this.createdSpecialsMeals();

	}

	public ArrayList<MenuOrder> getUserElectionOrder() {
	...
	public double calculateFinalPriceTotal() {
	...
	private void seeAdvMessage(int res, int... idMenuElectionUser) {
		...
		if (res == -4)
			System.out.println(
					"El id del plato ha sido seleccionado. Por favor ve al Cambiar Cantidad de platos para actualizar. \n");
		if (res == -5)
